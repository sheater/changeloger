...
async function run () {
	...
	if (!versions.length) {
		console.log(highlight.warning('There are no version tags.'));
	...
	for (let i = 0; i < limit; i++) {
		...
		const commits = await git.getCommits({ path, fromTag, toTag });
	...
	console.log(highlight.success(`successfully written to "${options.output}"`));
}
