...
async function getCommits ({ path, fromTag, toTag }, options) {
	let revision = `${toTag}`;
	...
	if (options.verbose) {
		console.log(highlight.debug(`revision ${revision}`));
	}
	...
	if (options.verbose) {
		console.log(highlight.debug(`${commits.length} commits was found`));
	}
